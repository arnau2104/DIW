    ...
    $(this).find(".column").each(function() {
      ...
      $(this).children(".element").each(async function() {
        if ($(this).find("p").length) {
          ...
          // console.log($(".content-image")[0].files[0]);
...
$(function() {
  ...
  $("#save-config").on("click", function() {
    ...
    dbNews.sort((a, b)=> +a[0].replace("news","") - +b[0].replace("news",""));
    ...
    if($("#image-input")[0].files[0]) {
      getBase64FromFile($("#image-input")[0].files[0],function (base64){
        ...
        console.log(`news${thisNewsExist ? newsId : +lastNewsId + 1}`)
         saveNews(thisNewsExist ? newsId : `news${+lastNewsId + 1}`,newsTitle, user[1].name,today,JSON.stringify(rows),principalImage,"0")
  
         setTimeout(()=>window.location.href="../pages/noticies.html",1000);
      })
    }else {
      console.log(`news${thisNewsExist ? newsId : +lastNewsId + 1}`)
      saveNews(thisNewsExist ? newsId : `news${+lastNewsId + 1}`,newsTitle, user[1].name,today,JSON.stringify(rows),activeNews[1].news_cover,"0")
      ...
      // console.log(dbNews);
        ...
        if($("#image-input")[0].files[0]) {
          getBase64FromFile($("#image-input")[0].files[0],function (base64){
            principalImage = base64;
            saveNewsInVariable(); 
            let lastNewsId = dbNews.length > 0 ? dbNews[dbNews.length - 1][0].replace("news","") : 0;         
                    
          console.log(lastNewsId); 
            saveNews(thisNewsExist ? newsId : `news${+lastNewsId + 1}`,newsTitle, user[1].name,today,JSON.stringify(rows),principalImage,"1")
